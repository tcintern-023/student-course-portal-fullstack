...
async function seed() {
  ...
  try {
    ...
    for (const user of testUsers) {
      ...
      userEmailToId[rows[0].email] = rows[0].id;
    }

    // Link the demo student account to a `students` row so ownership-based
    // authorization (see middleware/ownership.js) has something to
    // demonstrate: log in as student@studenthub.com and you can only ever
    // edit this one profile / manage this one set of enrollments.
    console.log("Linking demo student profile...");
    const { rows: demoStudentRows } = await client.query(
      `INSERT INTO students (name, email, user_id)
       VALUES ($1, $2, $3)
       ON CONFLICT (email) DO UPDATE SET user_id = EXCLUDED.user_id
       RETURNING id`,
      ["Test Student", "student@studenthub.com", userEmailToId["student@studenthub.com"]]
    );
    const demoStudentId = demoStudentRows[0].id;
    ...
    console.log("Enrolling demo student in a course...");
    await client.query(
      `INSERT INTO enrollments (student_id, course_id)
       SELECT $1, id FROM courses WHERE slug = 'web-development'
